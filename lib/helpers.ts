import { Unarray, ZendeskProperty, ZendeskObject, ZendeskIndexable, IndexableActionPath } from "./wrapper_types"

export function newIndexable<T extends any[]>(
    name: string, 
    construct: (identifier: string) => Unarray<T> extends string | boolean | number ? ZendeskProperty<Unarray<T>> : ZendeskObject<Unarray<T>>,
    ): ZendeskIndexable<T> {
...
export function newIndexableActionPath<T>(indexable: IndexableActionPath<T>): IndexableActionPath<T>  {
