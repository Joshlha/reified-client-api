import { ZendeskEvent, ZendeskObjectPath, ZendeskWritableObject, ZendeskWritableProperty, ZendeskAction } from "./wrapper_types"
...
export default class ReifiedClient {
  ...
  async get<T>(propOrProps: ZendeskObjectPath<T> | ZendeskObjectPath<any>[]) {
    if (Array.isArray(propOrProps)) {
      return await this.zafClient.get(propOrProps.map((item) => item._path))
    } else {
      return (await this.zafClient.get(propOrProps._path))[propOrProps._path] as T
    }
    
  }

  async set<T, W>(propOrObject: ZendeskWritableObject<T, W> | ZendeskWritableProperty<T, W>, arg: W): Promise<T> {
    return (await this.zafClient.set(propOrObject._path, arg))[propOrObject._path] as T
  }

  async invoke<F extends (...args: any) => any>(action: ZendeskAction<F>, ...args: Parameters<F>): Promise<ReturnType<F>> {
